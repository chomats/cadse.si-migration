...
public class NoPartParent extends Condition	 {
	MigrationModel model;
	
	public NoPartParent(MigrationModel model) {
		this.model = model;
	}
	...
	public List match(List listvalues) {
		...
		for (Object aVal : listvalues) {
			if (aVal instanceof ItemDescription) {
				if (hasNoPartParent((ItemDescription) aVal))
					ret.add(aVal);
			}
		}
		return ret;
	...
	private boolean hasNoPartParent(ItemDescription desc) {
